<template>
  <f7-page style="padding-bottom:45px" name="confirmdelivery"
  ...
  >
    ...
    <div style="margin-top:110px" v-if="page == 'success'">
      ...
      <div class="check_mark">
        ...
        <p style="color:green;font-size:22px;font-weight:700">Rs.{{cashback}}</p>
      <h3 style="color:black">Cashback Added to Wallet</h3>
      <h3 style="color:black">Order Successfully placed</h3>
      <f7-button href="/wallet" fill style="background-color:#00E676;line-height:50px;height:50px;font-size:14px;font-weight:750;border-radius:5px;margin:0px 15px">View Wallet <f7-icon f7="chevron_right" style="margin:-5px 0px 0px 10px"></f7-icon></f7-button>
      <f7-button href="/hdorders/" fill style="background-color:#00E676;line-height:50px;height:50px;font-size:14px;font-weight:750;border-radius:5px;margin:20px 15px 0px 15px">View Order <f7-icon f7="chevron_right" style="margin:-5px 0px 0px 10px"></f7-icon></f7-button>
      <f7-button href="/home" fill style="background-color:#00E676;height:50px;line-height:50px;font-size:14px;font-weight:750;border-radius:5px;margin:20px 15px 0px 15px">Continue Shopping <f7-icon f7="chevron_right" style="margin:-5px 0px 0px 10px"></f7-icon></f7-button>
      </center>
    </div>
    <div v-if="page == 'main'">
    ...
    <div v-if="!preloader">
      <f7-block-title  style="margin-top:15px"><h5 class="mpzero">ITEMS ORDERED (<span v-if="order.length != 0">{{ order.product.length }}</span>)</h5></f7-block-title>
    <f7-block v-if="order.product.length != 0" style="padding:0px;margin:0px 10px 0px 10px">
      ...
      <f7-card style="margin-bottom:10px">
          <f7-card-content style="padding:10px 0px">
              <f7-block style="margin:0px">
                <f7-row no-gap>
                  ...
                  <p class="mpzero"><span style="color:#616161">Rs.{{ product.selling_price }} </span><span style="color:gray;text-decoration:line-through">Rs.{{ product.mrp_price }}</span></p>
                  <p class="mpzero">Size : {{ product.size }}</p>
                  <p class="mpzero">qty : {{ product.qty }}</p>
      ...
      <f7-card style="padding:0px;margin:0px">
      <div style="padding:15px 10px;color:gray">
        <f7-row>
          ...
          <p class="mpzero" style="padding-bottom:8px">RS.{{total_mrp}}</p>
          <p class="mpzero" style="padding-bottom:8px;color:#FD6A02">- RS.{{total_discount}}</p>
        ...
        <f7-row style="color:rgb(97, 97, 97);font-weight:800">
         ...
         <f7-col style="text-align:right">
         <p class="mpzero" style="padding-top:8px">RS.{{total_selling}}</p>
         </f7-col>
        </f7-row>
        </div>
      </f7-card>
      ...
      <f7-block style="padding-top:0px;margin:16px 0px">
       <f7-card style="padding:0px;margin:0px">
        <f7-list style="margin:0px">
          <f7-list-item checkbox value="check_1" style="color:green" :after="'Rs.'+money" title="Use FS Wallet" @change="checkWallet"></f7-list-item>
        </f7-list>
      ...
      <div style="padding:15px 10px;color:gray">
        <f7-row>
          ...
          <f7-col style="text-align:right">
          <p class="mpzero" style="padding-bottom:8px">RS.{{total_selling}}</p>
          <p class="mpzero" style="padding-bottom:8px;color:#FD6A02">- RS.{{money}}</p>
          </f7-col>
        </f7-row>
        <hr>
        <f7-row style="color:rgb(97, 97, 97);font-weight:800">
         ...
         <f7-col style="text-align:right">
         <p class="mpzero" style="padding-top:8px">RS.{{total}}</p>
         </f7-col>
        </f7-row>
        </div>
        </f7-block>
    ...
    <f7-block-title  style="margin-top:15px"><h5 class="mpzero">DELIVERY ADDRESS</h5></f7-block-title>
      <f7-block style="padding:0px;margin:0px 10px 0px 10px">
      <f7-card v-for="address in order.address" :key="address.id">
                  <f7-card-content style="margin:0px;padding:10px 10px">
                      <h4 class="mpzero" style="color:#757575">{{ address.first_name }} {{ address.last_name }}</h4>
                  <h5 class="mpzero">{{ address.address }}</h5>
                  <h5 class="mpzero">{{ address.area }}</h5>
                  <h5 class="mpzero">Mobile : {{ address.mobile }}</h5>
                  </f7-card-content>
              </f7-card>
      </f7-block>
      <f7-block-title  style="margin-top:15px"><h5 class="mpzero">ESTIMATED DELIVERY</h5></f7-block-title>
      <f7-block style="padding:0px;margin:0px 10px 15px 10px">
      <f7-card>
                  <f7-card-content style="margin:0px;padding:10px 10px">
                      <h5 class="mpzero" style="color:#00E676">
                      Deliver By : Within 4 hours
                      </h5>
                  </f7-card-content>
              </f7-card>
      </f7-block>
      <f7-button type="button" :disabled="disabled" @click="placedOrder" fill style="background-color:#00E676;font-size:15px;font-weight:750;border-radius:0px;position:fixed;bottom:0;z-index:999;width:100%;height:45px;line-height:45px;">CONFIRM DELIVERY <f7-icon f7="chevron_right" style="margin:-5px 0px 0px 10px"></f7-icon></f7-button>
    </div></div>
    </div> </f7-page>
</template>
