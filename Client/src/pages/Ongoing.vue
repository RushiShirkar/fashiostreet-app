      ...
      <div v-show="!preloader">
	    <f7-list no-hairlines style="margin:12px 8px">
  			<template v-for="(result,index) in shopinfo">
	      			<f7-card style="margin:12px 0px;padding:10px 16px;background-color:#FFFFFF-webkit-box-shadow:none;box-shadow:none;border-radius:4px;border:1px solid gray" v-show="result.offers!=null">
					    ...
					    <f7-list no-hairlines style="margin:0px;width:100%;">
					    	<template v-for="offer in result.offers">
					    		<p style="padding:8px;margin-top:8px;background-color:skyblue;color:navy;text-align: center;font-size:18px;font-weight: bold;border-radius:3px">{{offer}}</p>
							</template>
							<h4 style="color:#000000;margin:5px 0px 8px 0px"><img src="../assets/shop_icon.png" style="width:30px;height:30px;border-radius:50%;vertical-align: middle;" alt="Shops">&nbsp {{result.name}}</h4>
						</f7-list>
	    			</f7-link>
	    			</f7-card>
        	</template>
